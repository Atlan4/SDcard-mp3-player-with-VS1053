 ...
 * \ beta verzia v2.24, working on atmega2560
 ...
 */
vs1053 MP3player;
unsigned int last_ms_char;  // milliseconds of last recieved character from Serial port.
char buffer_pos;            // next position to recieve character from Serial port.
unsigned int fileindir;     // file in dir, "l" command, for playAll
unsigned char PlayAll;      // PlayAll random file to dir
unsigned char PlayAllR;     // PlayAll random file to dir
 ...
 */
  ...
  char dirbuffer[75];       // for name of directory temporary
  char dirbuffer2[75]="/";  // for name of directory
...
void setup() {
 ...
 //     MP3player.setVolume(70,70); // commit new volume headphones
/*
#if (0)
...
//  parse_menu('j'); // display the list of files to play
}
          ...
          while (file.openNext(&dir, O_RDONLY))
            {
              ...
              SerialPrintPaddedNumber(count, 3 );
              Serial.println(F(""));
...
//--------------------------------------------------------------------
// play RANDOM all files to directory
      if(!MP3player.isPlaying()&&(PlayAllR)) {
        delay(100);    //bolo 100
        Serial.print(F("Files in directory:"));
        Serial.println(fileindir);
        PlayAllR=random(1,fileindir);
        //if (PlayAll<=fileindir)
                  
          Serial.print(F("Play all files in dir random, play file number:"));
          Serial.println(PlayAllR);
          sprintf (buffer, "%02i", PlayAllR);
          //Serial.println(buffer);
          buffer_pos = 2;
          //PlayAll++;
          //}else {PlayAll=0;buffer_pos = 0;}  //end play all file to dir
...
//*************************************************************************
  ...
  Serial.println(F("Play RANDOM ALL files to dir"));
  PlayAllR=1;
...
void help() {
  ...
  Serial.println(F(" [j] Play RANDOM ALL files to dir"));
...
//  Serial.println(F(" [> or <] to increment or decrement play speed by 1 factor"));
  ...
  Serial.println(F(" [0###] Enter index of file to play, zero pad! e.g. 01-999"));
