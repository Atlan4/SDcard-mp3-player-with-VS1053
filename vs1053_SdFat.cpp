...
uint8_t vs1053::playMP3(char* fileName, uint32_t timecode) {
  ...
  //Open the file in read mode.
  if(!track.open(fileName, O_RDONLY)) return 2;
  ...
  delay(100); // skusil som 10 experimentally found that we need to let this settle before sending data.
...
//------------------------------------------------------------------------------
//*******************************************************************************
void vs1053::trackID3(char* infobuffer){  
...
//********* pause  payback go read ID3 info *******************

  //skip to start
  track.seekSet(0);

  //read 4 byte to start
  track.read(infobuffer, 4);          //nacitaj prve 4 bajty
  char datStr[] = "ID3";
    for (byte i=0; i<3; i++){       //4 tagSize
      if(infobuffer[i] != datStr[i]){
        ParseID3_v1(); break;
        }else if(i==2){
        //found = 1;
        //-------------------------------------------------------------------------  
        //ID3 found,test v2 or v3 version               
        if (infobuffer[3] == 0x02) 
            {
            ParseID3_v2();            //ParseID3_v2_2(file, ID3);
            }
        else if (infobuffer[3] == 0x03 || infobuffer[3] == 0x04)
                {
                ParseID3_v3();      //ParseID3_v2_3(file, ID3);
                }
  
            Serial.write(infobuffer, 3);
            Serial.write("v2.");            
            Serial.write(infobuffer[3]+48);
            Serial.println();       
...
//------------------------------------------------------------------------------
void vs1053::ParseID3_v1(){

  Serial.print(F("ID3v1"));
  Serial.println();
  char infobuffer[30];
  memset(infobuffer,  0, sizeof(infobuffer));
 // char infobufferUTF8[60];
  uint8_t i;
  //skip to end
  track.seekEnd(-125);    //-128+3
  for(i=0;i<3;i++){
                  //read 30 bytes of tag informat at -128 + offset
                  track.read(infobuffer, 30);          //bolo 30
                  //for(a=0;a<30;a++){if((infobuffer[a])==0) break;}
                  if (i==0)Serial.print(F("Title: "));
                  if (i==1)Serial.print(F("Artist: "));
                  if (i==2)Serial.print(F("Album: ")); 

                  Latin1toUTF8(infobuffer,30);
                  }             

}
//********************************************************************************************
void vs1053::Latin1toUTF8(char *buffer, unsigned char size){

  char infobufferUTF8[100];
  memset(infobufferUTF8,  0, sizeof(infobufferUTF8));
  char d=0;
  unsigned char a,b,x; 
  for(a=0;a<size;a++){if((buffer[a])==0) break;}     
  for(b=0;b<a;b++)
    {
    x=buffer[b];
    if (x<0x7F)//{if (x==0x69) {infobufferUTF8[d++]=0XC2;infobufferUTF8[d++]=0X9E;} else 
               //{if (x==0x69) {infobufferUTF8[d++]=0XC2;infobufferUTF8[d++]=0X9E;} else
                    {infobufferUTF8[d++]=buffer[b];}
               //}
    else {
          if (x==0xA9){infobufferUTF8[d++]=0xC5;infobufferUTF8[d++]=0XA0;} //Š
          ...
          if (x==0xCD){infobufferUTF8[d++]=0XC4;infobufferUTF8[d++]=0XBA;} //Í
          if (x==0xCF){infobufferUTF8[d++]=0XC4;infobufferUTF8[d++]=0X8E;} //Ď
          if (x==0xD2){infobufferUTF8[d++]=0XC5;infobufferUTF8[d++]=0X87;} //Ň
          if (x==0xDA){infobufferUTF8[d++]=0XC3;infobufferUTF8[d++]=0X9A;} //Ú
          if (x==0xDD){infobufferUTF8[d++]=0XC3;infobufferUTF8[d++]=0X9D;} //Ý
          if (x==0xE1){infobufferUTF8[d++]=0XC3;infobufferUTF8[d++]=0XA1;} //á
          if (x==0xE4){infobufferUTF8[d++]=0XC3;infobufferUTF8[d++]=0XA4;} //ä
          if (x==0xE8){infobufferUTF8[d++]=0XC4;infobufferUTF8[d++]=0X8D;} //č
          if (x==0xE9){infobufferUTF8[d++]=0XC3;infobufferUTF8[d++]=0XA9;} //é
          if (x==0xEC){infobufferUTF8[d++]=0XC4;infobufferUTF8[d++]=0X9B;} //ě
          if (x==0xED){infobufferUTF8[d++]=0XC3;infobufferUTF8[d++]=0XAD;} //í
  ...
  Serial.write(infobufferUTF8, d);
  Serial.println();
                  //Serial.write(infobuffer, 3);
}
//------------------------------------------------------------------------------
void vs1053::ParseID3_v3(){
  ...
  char itrack[2];
  char iyear[4];
  memset(ititle,  0, sizeof(ititle));
  memset(iartist, 0, sizeof(iartist));
  memset(ialbum,  0, sizeof(ialbum));
  memset(itrack,  0, sizeof(itrack));  
  memset(iyear,   0, sizeof(iyear));  
  ...
  track.seekSet(10);
  //read OD 10 BAJTU
  track.read(infobuffer,300);          //nacitaj 300 bajty

  for(i=0;i<300;i++){if(infobuffer[i]==0)infobuffer[i] = 32;}
  //Serial.write(infobuffer,300);

  adr=strstr(infobuffer,"TIT2");      // ak nenájde vráti NULL inak adresu začiatku
  if(adr){  
          frame=adr[7];
          for (i=0;i<(frame-1);i++)
          ititle[i]=adr[i+11];
  ...
  adr=strstr(infobuffer,"TPE1");      // ak nenájde vráti NULL inak adresu začiatku 
  if(adr){
          frame=adr[7];
          for (i=0;i<(frame-1);i++)
          iartist[i]=adr[i+11];
          }
  adr=strstr(infobuffer,"TALB");      // ak nenájde vráti NULL inak adresu začiatku 
  if(adr){
          frame=adr[7];
          for (i=0;i<(frame-1);i++)
          ialbum[i]=adr[i+11];
          }
  adr=strstr(infobuffer,"TRCK");      // ak nenájde vráti NULL inak adresu začiatku
  if(adr){ 
          frame=adr[7];
          for (i=0;i<(frame-1);i++)
          itrack[i]=adr[i+11];
          }
  adr=strstr(infobuffer,"TYER");      // ak nenájde vráti NULL inak adresu začiatku 
  if(adr){
          frame=adr[7];
          for (i=0;i<(frame-1);i++)
          iyear[i]=adr[i+11];
  ...
  if(j==1) ParseID3_v1(); else //ak sa nenajde TIT2 pouzi ID3v1 
          {                
          Serial.print(F("Album: "));

          Latin1toUTF8(ialbum,50);                        
          //Serial.write(ialbum,50);
          //Serial.println();
          Serial.print(F("Artist: "));
          Latin1toUTF8(iartist,40);
          //Serial.write(iartist,40);
          //Serial.println();
          Serial.print(F("Title: "));
          Latin1toUTF8(ititle,50);
          //Serial.write(ititle,50);
          //Serial.println();
          Serial.print(F("Track: "));
          Latin1toUTF8(itrack,2);
          //Serial.write(itrack,2);
          //Serial.println();
          Serial.print(F("Year: "));
          Latin1toUTF8(iyear,4);
          //Serial.write(iyear,4);
          //Serial.println(); 
          }  
}
//------------------------------------------------------------------------------
void vs1053::ParseID3_v2(){
  ...
  adr=strstr(infobuffer,"TT2");      // ak nenájde vráti NULL inak adresu začiatku
  if(adr){  
          frame=adr[5];
          for (i=0;i<(frame-1);i++)
          ititle[i]=adr[i+7];
  ...
  adr=strstr(infobuffer,"TP1");      // ak nenájde vráti NULL inak adresu začiatku 
  if(adr){
          frame=adr[5];
          for (i=0;i<(frame-1);i++)
          iartist[i]=adr[i+7];
          }
  adr=strstr(infobuffer,"TAL");      // ak nenájde vráti NULL inak adresu začiatku 
  if(adr){
          frame=adr[5];
          for (i=0;i<(frame-1);i++)
          ialbum[i]=adr[i+7];
          }
  adr=strstr(infobuffer,"TRK");      // ak nenájde vráti NULL inak adresu začiatku
  if(adr){ 
          frame=adr[5];
          for (i=0;i<(frame-1);i++)
          itrack[i]=adr[i+7];
          }
  adr=strstr(infobuffer,"TYE");      // ak nenájde vráti NULL inak adresu začiatku 
  if(adr){
          frame=adr[5];
          for (i=0;i<(frame-1);i++)
          iyear[i]=adr[i+7];
  ...
  if(j==1) ParseID3_v1(); else //ak sa nenajde TT2 pouzi ID3v1 
          {                
          Serial.print(F("Album: "));
          Latin1toUTF8(ialbum,50);                          
          //Serial.write(ialbum,50);
          ...
          //Serial.println();
          Serial.print(F("Track: "));
          Latin1toUTF8(itrack,5);
          ///Serial.write(itrack,5);
          //Serial.println();
          Serial.print(F("Year: "));
          Latin1toUTF8(iyear,4);
          //Serial.write(iyear,4);
