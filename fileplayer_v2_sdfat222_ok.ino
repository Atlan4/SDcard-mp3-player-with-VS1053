 ...
 * \ beta verzia, working on atmega2560
 ...
 */
  ...
  char dirbuffer[60]; // for name of directory
//   char dirbuffer2[60]="/Ceske hity/"; // for name of directory
  char dirbuffer2[60]="/Vikki Carr - The First Time Ever I Saw Your Face/"; // for name of directory
 // char dirbuffer2[60]="/"; // for name of directory
  //char dirbuffer2[60]="/test/"; // for name of directory
...
// Below is only needed if not interrupt driven. Safe to remove if not using.
  ...
  //-------------------------------------------------------------------------
      ...
      {
        if (fn_index==10000){
        memset(dirbuffer2, 0, sizeof(dirbuffer2));
//v1        dirbuffer2[0]='/';Serial.println(dirbuffer2);sd.chdir((dirbuffer2),true);
//       dirbuffer2[0]='/';Serial.println(dirbuffer2);sd.chdir(dirbuffer2);dir.open(dirbuffer2);dir.rewind();
                dirbuffer2[0]='/';Serial.println(dirbuffer2);sd.chdir(dirbuffer2);dir.open(dirbuffer2);
                }else{
...
//      memset(dirbuffer2, 0, sizeof(dirbuffer2));
 ...
 //     String(dirbuffer)=("/Ceske hity/");
      Serial.println(dirbuffer);
      dirbuffer.toCharArray(dirbuffer2,60);
      Serial.println(dirbuffer2);
      File dir;
        if (!sd.chdir(dirbuffer2)) {
    error("sdchdir failed");
  }
  if (!dir.open(dirbuffer2)) {
    error("dir.open failed");
  }
 /*   if (!dir.rewind()) {
    error("dir.rewind failed");
  }  
  */
   /*   sd.chdir(dirbuffer3);
      dir.open(dirbuffer3);
      dir.rewind();
      */
      Serial.print(F("zmenilo < adresar? "));
        }
      }else
      {
...
//v1      while (file.openNext(sd.vwd(),O_READ))
      while (file.openNext(&dir, O_RDONLY))
      {
 ...
 //           Serial.println(filename);
            ...
            */
            ...
            /*
            uint8_t a;
            //for(a=0;a<8;a++){if((ID[a])==0) break;}
            Serial.write((byte*)&ID, 3);
            Serial.println();
            */
            /*
            ...
            */
            ...
            uint8_t a;
            for(a=0;a<30;a++){if((title[a])==0) break;}
            Serial.write((byte*)&title, a);
            Serial.println();
            Serial.print(F("by:  "));
            for(a=0;a<30;a++){if((artist[a])==0) break;}
            Serial.write((byte*)&artist, a);
            Serial.println();
            Serial.print(F("Album:  "));
            for(a=0;a<30;a++){if((album[a])==0) break;}
            Serial.write((byte*)&album, a);
            Serial.println();
            */
            break;
          }
...
//      sd.ls(LS_R | LS_DATE | LS_SIZE);
 //sd.ls("/"); //korenovy adresar  //v1 docasne
 //v1         sd.ls(dirbuffer2,true); 
                  sd.ls(dirbuffer2); 
...
//v1      sd.chdir(dirbuffer2,true); 
      ...
      dir.open(dirbuffer2);
      dir.rewind();
    //  sd.chdir("/Vikki Carr - The First Time Ever I Saw Your Face");
 //     dir.open(dirbuffer2); 
      uint16_t count = 10001;
...
void help() {
  ...
  Serial.println(F(" [q] Find folders"));
